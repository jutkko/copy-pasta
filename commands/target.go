...
func (t *TargetCommand) Run(args []string) int {
	...
	if len(args) > 0 {
		if target, ok := config.Targets[args[0]]; ok {
			if err := runcommands.Update(target.Name, target.Backend, target.AccessKey, target.SecretAccessKey, target.BucketName, target.Endpoint, target.Location); err != nil {
